...
class Roll extends React.Component {
  ...
  // Function to update the state - packSize - whenver the user makes a change from the dropdown - and store it in Number format
  setPackSize = (event) => {
    this.setState({
  ...
  updateSelectedClassName = (currentValue) => {
    return (this.state.packSize === currentValue) ? 'selected-packsize' : '';
  }

  buttonStyle = (currentValue) => {
    return {
      backgroundColor: (this.state.packSize === currentValue) ? '#D3D3D3' : 'white'
    };
  }
